...
| JWST early galaxies | `M★` vs `M_halo·ε` | photometry→SED (IMF+SFH+dust posits) vs ΛCDM HMF + efficiency posit | ทางหนีเปิดพร้อมกัน**หลาย posit อิสระ**: ฝั่ง M★ — IMF posit เป็น load-bearing และขยับได้ทั้งสองทิศ (bottom-heavy เพิ่มมวล 3–4× ทำให้แย่ลง: 2601.20864); ฝั่ง M_halo — เปลี่ยน halo mass function/power spectrum ก็ดับ tension ได้ทั้งก้อนโดยไม่แตะ IMF (2507.23742) — การที่คู่ชนดับได้จากคนละฝั่งด้วยคนละ posit คือลายเซ็นว่ามันเป็น posit-vs-posit ไม่ใช่โลก-vs-โลก; บวก Π ใหม่สุดขั้ว + "little red dots"→AGN ปน | arXiv:2601.20864 (IMF, ฝั่ง M★) + 2507.23742 (HMF, ฝั่ง M_halo — คนละ posit, ระบุชัด); gates run 2026-07-19 |

หมายเหตุเชิงวิธี: ทั้งสี่เคสถูกไล่ผ่าน `[removed-internal-engine]` จริง (G3/G4/G5 FLAG ตาม
